...
class HandPose:
    ...
    def __init__(self, input_tensor, netout, netscale, net_var_list):
        ...
        self.net_train_var_list = net_var_list
        ###
        self.is_pred_init = False
        ###
        self.joint_amount = self.netout.shape[-1].value - 1

    def load_weights(self, tfckpt_path):
        saver = tf.train.Saver(var_list=self.net_train_var_list)
    ...
    def init_predict_model(self):
        with tf.variable_scope('CWZHandPosePredict'):
            gaussian_heatMat = tf.nn.depthwise_conv2d(
                self.netout, 
                make_gauss_var('gaussian_filter', self.netout.get_shape()[3].value), 
